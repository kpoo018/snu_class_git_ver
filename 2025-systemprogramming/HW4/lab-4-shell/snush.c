...
static void check_bg_status() {
    ...
    while (curr) {
        ...
        printf("[%d] Process group: %d done\n", curr->job_id, curr->pgid);

        // 리스트에서 언링크
        manager->done_bg_jobs = next;
...
static void sigchld_handler(int signo) {
    ...
    if (signo == SIGCHLD) {
        while((pid = waitpid(-1, &stat, WNOHANG)) > 0) {
            ...
            while (curr) {
                for (int i = 0; i < curr->total_num; i++) {
                    if (curr->pid_list[i] == pid) {
                        curr->curr_num--;
                        break;
                    }
                }
                ...
                if (curr->curr_num == 0) {
                    ...
                    if (prev)
                        ...
                        manager->jobs = curr->next;

                    manager->n_jobs--;

                    if (curr->state == background) {
                        add_done_bg_job(curr);
                    } else {
                        free(curr->pid_list);
                        free(curr);
                    }
