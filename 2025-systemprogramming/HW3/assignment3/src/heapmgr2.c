...
/* g_free_heads: each bin contains the chunks exponetially increased range of size 
* 0-1, 2-3, 4-7, 8-15, 16-31, 32-63, 64-127, 128-255, 256-511, 512+.
* g_free_heads[0] is for size 0-1, g_free_heads[1] is for size 2-3,
* g_free_heads[2] is for size 4-7, ... and g_free_heads[9] is for size 512+.
*/
static Chunk_T g_free_heads[max_bins] = {NULL};
...
check_heap_validity(void)
{
   ...
   for(int i=0; i<max_bins; i++){
      for (w = g_free_heads[i]; w; w = chunk_get_ptr(w)) {
         Chunk_T n;
         ...
         if(chunk_get_ptr(w) == g_free_heads[i])
            break;
      }
   }
...
/*--------------------------------------------------------------*/
/* get_bin_num:
 * Returns the bin number for the given size in units.
 * The bin number is used to find the proper free chunk list. 
 * The bin number is 0 for size 1, 1 for size 2-3, 2 for size 4-7,
 * ... and 9 for size 512-
 */
/*--------------------------------------------------------------*/
static int
get_bin_num(size_t units) {
   int i = 0;
   if (units == 0) return -1; // Handle zero input gracefully
   while (units > 1) {
      units >>= 1; // Use bit-shifting for efficiency
      i++;
      if (i >= max_bins - 1)
         return max_bins - 1;
   }
...
insert_chunk_to_free_head(Chunk_T c)
{
   ...
   Chunk_T g_free_head = g_free_heads[get_bin_num(chunk_get_units(c))];
   assert(g_free_head == NULL || chunk_get_status(g_free_head) == CHUNK_FREE);

   /* If the freelist is empty, chunk c points to itself. */
   if (g_free_head == NULL) {
      chunk_set_ptr(c, c);
      chunk_set_ptr(c_footer, c);

      g_free_heads[get_bin_num(chunk_get_units(c))] = c;
   }
   else {
...
remove_chunk_from_list(Chunk_T c)
{
   ...
   if (prev == c && next == c) { 
      /* c is the only chunk in the free chunk list */
      g_free_heads[get_bin_num(chunk_get_units(c))] = NULL;
      ...
      if (g_free_head == c) {
         assert (next != c);
         g_free_heads[get_bin_num(chunk_get_units(c))] = next;
      }
   }
 ...
 * chunk. 'prev' should be the last chunk in the free chunk list.
...
allocate_more_memory(size_t units)
{
   ...
   /* Insert the newly allocated chunk 'c' to the free chunk list. */
   c = insert_chunk(c);
...
heapmgr_malloc(size_t size)
{
   ...
   for(int i = get_bin_num(units); i < max_bins; i++){
      Chunk_T g_free_head = g_free_heads[i];
      for (c=g_free_head; 
         c; 
         c = chunk_get_ptr(c)) {
