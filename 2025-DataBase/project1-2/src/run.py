...
class MyTransformer(lark.Transformer):
    ...
    def drop_table_query(self, items):
        ...
        try:
            ...
            self._is_table_referenced(table_id, table_name, meta_db, txn)
    ...
    def delete_query(self, items):
        ...
        try:
            ...
            if flag:
                txn.abort()
                prompt()
                print(f"{cnt} row(s) are not deleted due to referential integrity")
                table_db.close()
                meta_db.close()
            else:
                txn.commit()
            ...
            txn.abort()
            meta_db.close()
            raise MyTransformerError(f"Delete table has failed: {e}")
    ...
    def select_query(self, items):
        ...
        try:
            ...
            if select_list.children == []:
                ...
                for selected_column in select_list.find_data("selected_column"):
                    if selected_column.children[0] == None:
                        ...
                        try:
                            column_name = self._check_ambiguous_column(column_name, joined_schema_column_types)
                        ...
                        if table_name not in {col.split(".")[0] for col in joined_schema_column_types.keys()}:
                            raise MyTransformerError(f"'{table_name}' does not exist")
                        column_name = selected_column.children[1].children[0].lower()
                        column_name = f"{table_name}.{column_name}"
                        if column_name not in joined_schema_column_types.keys():
            ...
            if order_by_clause:
                ...
                if table_name:
                    if table_name.children[0].lower() not in {col.split(".")[0] for col in joined_schema_column_types.keys()}:
                ...
                column_type = joined_schema_column_types[column_name]
                column_idx = list(joined_schema_column_types.keys()).index(column_name)

                reverse = False
                if order_by_clause.children[4]:
                    order_token = order_by_clause.children[4].children[0].lower()
                    reverse = (order_token == 'desc')

                # 정렬 수행
                if column_type == "int":
                    joined_records.sort(key=lambda x: int(x[column_idx]), reverse=reverse)
                else:
                    joined_records.sort(key=lambda x: x[column_idx], reverse=reverse)
    ...
    def _join_tables(self, from_clause, meta_db, txn):
        ...
        if len(join_clause.children) > 1:
            ...
            second_table_id = self._get_table_id(second_table_name, meta_db, txn)
            second_schema_key = f"__schema__:{second_table_id}".encode()
            second_schema_value = meta_db.get(second_schema_key, txn=txn)
            if not second_schema_value:
                raise MyTransformerError("Fatal Error: Table ID exist, but schema does not exist.")
            
            second_schema_column_types = second_schema_value.decode().split("|")
            second_schema_column_types = {col.split(":")[0]: col.split(":")[1] for col in second_schema_column_types}

            second_table_db = db.DB(self.db_env)
            second_table_db.open(second_table_id, None, dbtype=db.DB_HASH, txn=txn)

            second_cursor = second_table_db.cursor(txn=txn)
            second_records = []
            while record := second_cursor.next():
                second_records.append(record[1].decode().split("|"))
            second_cursor.close()
            second_table_db.close()
    ...
    def _perform_join(self, table1_schema, table2_schema, records, second_records, join_clause, n, meta_db, txn):
        ...
        if len(join_clause.children) <= n:
            ...
            next_table_id = self._get_table_id(next_table_name, meta_db, txn)
            next_schema_key = f"__schema__:{next_table_id}".encode()
            next_schema_value = meta_db.get(next_schema_key, txn=txn)
            if not next_schema_value:
                raise MyTransformerError("Fatal Error: Table ID exist, but schema does not exist.")
            
            next_schema_column_types = next_schema_value.decode().split("|")
            next_schema_column_types = {col.split(":")[0]: col.split(":")[1] for col in next_schema_column_types}
            
            next_table_db = db.DB(self.db_env)
            next_table_db.open(next_table_id, None, dbtype=db.DB_HASH, txn=txn)
            
            next_cursor = next_table_db.cursor(txn=txn)
            next_records = []
            while record := next_cursor.next():
                next_records.append(record[1].decode().split("|"))
